from IHSetTurki import calibration, turki
...
model = calibration.cal_Turki(wrkDir+'/data_rot/', depth=10, angleBathy=36, D50=0.427e-3, BeachL=1200)
...
kk = best_model_run['parkk']

full_run, _ = turki.turki(model.EF, model.Hb, model.theb, model.BeachL, model.dt, kk, model.Obs[0])
# full_run, _ = turki.turki(model.EF, model.Hb, model.theb, model.BeachL, model.dt, kk, best_model_run['paralp0'])
...
ax[0].plot(model.time, full_run, color='red',linestyle='solid', label= 'Turki et al.(2013)')
...
ax[0].set_ylabel('Orientation [deg.]', fontdict=font)
...
ax[1].plot(model.time, model.EF/np.max(model.EF),color='black',linestyle='solid', label='Best objf.='+str(bestobjf))
ax[1].set_ylim([0,1])
ax[1].set_xlim([model.time[0], model.time[-1]])
ax[1].set_yticks([0, 1], ['0', r'$EF_{max}$'])
plt.subplots_adjust(hspace=0.3)
fig.savefig('./results/Turki_Best_modelrun_'+str(config.cal_alg.values)+'.png',dpi=300)
